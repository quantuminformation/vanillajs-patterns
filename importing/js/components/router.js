...
export default async (hostComponent, baseUrl = config.BASE_URL) => {
  ...
  const loadRoute = async (url) => {
    const normalizedUrl = sanitizePath(url);
    try {
      const routePath =
        normalizedUrl === '/' ? `${baseUrl}/routes/index.js` : `${baseUrl}/routes${normalizedUrl}.js`;
      ...
      updateActiveLink(normalizedUrl); // Update active link based on the route
    } catch (err) {
